...
export default function ProductRating({ rating }) {
  const handleWitdth = index => {
    const _index = index + 1 //vi index bat dau bang 0 nen phai cong them 1
    if (_index <= rating) {
      return '100%'
    }
    if (0 < _index - rating && _index - rating < 1) {
      // vi du 4.5 thi 4.5-4=0.5
      ...
      {Array(5)
        ...
        .map((item, index) => (
          <S.RatingStarWrapper key={index}>
            <S.RatingStarPercent style={{ width: handleWitdth(index) }}>
              <svg
